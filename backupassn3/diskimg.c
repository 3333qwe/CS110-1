...
foundInCache(int sectorNo,void *buf,int *size){
  ...
  /*for(SectorEntry *ptr = cacheMem; ptr < cacheMem + numEntries;
      ptr += 1){
    if(ptr->sectorNum == sectorNo && ptr->isAllocated){
      *size = ptr->size;
      memcpy(buf,&(ptr->buf),ptr->size);
...
fetch(int fd,int sectorNum, void *buf)
{
  ...
  /*
  // Increase the number of entries in the cache only if it's not full
  if(numEntriesInCache < maxEntries){
    numEntriesInCache++;
    }*/
  return entry.size;
}
