...
{
  /* Read the Query */
  // Read the query size. It is included as a header to the query
  char qSize[sizeof(int)];
  int totalBytesRead = 0;

  while(totalBytesRead < (int)sizeof(int)){
    char *buf = qSize + totalBytesRead;
    int bytesToRead = sizeof(int) - totalBytesRead;
    int nbytes = read(sock,buf,bytesToRead);
    if(nbytes < 0)perror("read");
    totalBytesRead += nbytes;
  }
  // Get the query Size as an integer
  int querySize;
  memcpy(&querySize,qSize,sizeof(int));

  // Read the word to query 
  char word[querySize+1];
  totalBytesRead = 0;
  while(totalBytesRead < querySize){
    int bytesToRead = querySize - totalBytesRead;
    char *buf = word + totalBytesRead;
    int nbytes = read(sock,buf,bytesToRead);
    if(nbytes < 0)perror("read");
    totalBytesRead += nbytes;
  }
  ...
  /* Query the word */
  char *result = NULL;
  FindWord(word, diskIndex, &result);
  int len = strlen(result);
 
  /* Write the response */
  // Copy the integer into an array of chars for writing
  char length[sizeof(int)];
  memcpy(length,&len,sizeof(int));

  // Write the length of the response
  int totalBytesWritten = 0;
  while(totalBytesWritten < (int)sizeof(int)){
    char *buf = length + totalBytesWritten;
    int bytesToWrite = sizeof(int) - totalBytesWritten;
    int nbytes = write(sock,buf,bytesToWrite);
  ...
  while(totalBytesWritten < len){
    char *buf = result + totalBytesWritten;
    int bytesToWrite = len - totalBytesWritten;
    int nbytes = write(sock,buf,bytesToWrite);
    if(nbytes < 0)perror("write");
    totalBytesWritten += nbytes;
  }
