...
typedef struct Report {
  ...
  // Number of calls to Report_DoReport
  int reportCalls;
} Report;
...
// An array of reports. One for each worker
static Report *reports;

/*
 * Setter for the reports fields
 */
void initializeReport(Report *report){
  report->numReports = 0;
  report->reportCalls = 0;
  for (int r = 0; r < MAX_NUM_REPORTS; r++){
    report->dailyData[r].hasOverflowed = 0;
    report->dailyData[r].numLogEntries = 0;
  }
}


/*
 * Takes all of the logs in the individual workers' reports
 * and tabulates them into the bank's report
 */
void joinReports(Report *bankReport){
  // The total number of reports across all of the workers
  int totalReports = bankReport->numReports; 
  // Iterate over all the workers
  for(int i=0; i < numWorkers; i++){
    Report *rpt = &reports[i];
    int numReports = rpt->numReports;
    // Copy the daily data for every worker into the bank's report
    for(int j=0; j < numReports; j++){
      memcpy(&bankReport->dailyData[totalReports],&rpt->dailyData[j],sizeof(rpt->dailyData[0]));
      totalReports++;
    }

  }
  // Set the bank's number of reports
  bankReport->numReports = totalReports;
}
...
Report_Init(Bank *bank, AccountAmount reportAmount, int maxNumWorkers)
{
  ...
  // Allocate the the array of reports
  reports = malloc(numWorkers * sizeof(Report));    

  // Initialize the static array of reports
  for(int i = 0; i < numWorkers;i++){
    initializeReport(&reports[i]);
  }
  
  // Initialize the bank's report
  initializeReport(bank->report);    
...
{	
  ...
  // Each worker will write into their own report structure
  Report *rpt = &reports[workerNum];  
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  // Acquire the report's lock to look at how many calls to DoReport have been made
  pthread_mutex_lock(&lock);
  rpt->reportCalls = rpt->reportCalls + 1;
    
  // Wait for all of the workers to call _DoReport
  while((rpt->reportCalls % numWorkers) != 0){
    pthread_cond_wait(&cond,&lock);
  	
    /*
     * Only the last worker will "do the report"
     * Rest will wait till the last worker calls doReport and then exit rightaway
     */
  ...
  // The last worker needs to tabulate all of transfer logs done by the
  // individual workers into the bank's report
  joinReports(rpt);
